...
onViewVar:boolean=false;
  ...
  private matchesPatientName(patient: Patient): boolean {
    ...
    return fullName.toLowerCase().includes(this.patientNameQuery.toLowerCase()) ||
  ...
  private matchesPatientFirstName(patient: Patient): boolean {
    ...
    return fullName.toLowerCase().includes(this.patientFirstNameQuery.toLowerCase()) ||
  ...
  private matchesPatientLastName(patient: Patient): boolean {
    ...
    return fullName.toLowerCase().includes(this.patientLastNameQuery.toLowerCase()) ||
           fullName.toLowerCase().includes(this.patientLastNameQuery.toLowerCase());
  }


  private matchesPatientDOB(patient: Patient): boolean {
    const studyDOB = new Date(patient.patientBirthDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
    return studyDOB === new Date(this.patientDobQuery).toLocaleDateString('en-GB');
  }
  ...
  private matchesGender(patient: Patient): boolean {
    ...
    return (patientGender === this.genders[0] && queryGender === "male") ||
  ...
  clearPatientLastName() {
    ...
    this.searchPatients();
              ...
              modify(patient:Patient){
                console.log(patient.patientStatus,PatientStatus.Valid)
                if (this.getPatientStatusIndex(patient.patientStatus) == PatientStatus.Valid){this.disablePatient(patient.patientKy)}
                else{this.enablePatient(patient.patientKy);}
              }
