...
export class PatientService {
  ...
  private archivedPatientsSubject = new BehaviorSubject<Patient[]>([]);
  archivedPatients: Observable<Patient[]> =
    this.archivedPatientsSubject.asObservable();
  ...
  getPatientByNameDOB(
    ...
    return this.http.get<Patient>(
      `${this.baseUrl}/getPatient/${firstName}/${lastName}/${dob}`
    );
  }

  updatePatient(patientKy: number, updatedPatient: any): Observable<any> {
    return this.http.put<Patient>(
      `${this.baseUrl}/updatePatient/${patientKy}`,
      updatedPatient
    );
  }
  ...
  deletePatient(patientKy: number): Observable<Patient> {
    return this.http.delete<Patient>(
      `${this.baseUrl}/deletePatient/${patientKy}`
    );
  }
  disablePatient(patientKy: number): Observable<any> {
    return this.http.put<Patient>(
      `${this.baseUrl}/disablePatient/${patientKy}`,
      {}
    );
  }
  ...
  enablePatient(patientKy: number): Observable<any> {
    return this.http.put<Patient>(
      `${this.baseUrl}/enablePatient/${patientKy}`,
      {}
    );
  }
  ...
  deleteAddress(addressKy: number, patientKy: number): Observable<any> {
    return this.http.delete<Address>(
      `${this.baseUrl}/deleteAddress/${addressKy}/${patientKy}`,
      {}
    );
  }
  // Method to delete a phone
  deletePhone(phoneKy: number, patientKy: number): Observable<any> {
    return this.http.delete<Phone>(
      `${this.baseUrl}/deletePhone/${phoneKy}/${patientKy}`,
      {}
    );
  }
  ...
  deleteEmail(emailKy: number, patientKy: number): Observable<any> {
    return this.http.delete<Email>(
      `${this.baseUrl}/deleteEmail/${emailKy}/${patientKy}`,
      {}
    );
  }

  archivePatient(patient: Patient): void {
    const currentArchived = this.archivedPatientsSubject.getValue();
    this.archivedPatientsSubject.next([...currentArchived, patient]);
  }

  getArchivedPatients(): Observable<Patient[]> {
    return this.archivedPatients;
  }
}
